...
namespace DUTY1000
{
    public partial class duty204y : SilkRoad.Form.Base.FormX
    {
        ...
        private void baseInfoSearch()
        {
			...
			df.GetSEARCH_CALLDatas(clib.DateToText(dat_yymm.DateTime).Substring(0, 6), dept, ds);
			...
			schedulerStorage1.Appointments.DataSource = ds.Tables["SEARCH_CALL"];
        ...
        private void duty204y_Load(object sender, EventArgs e)
        {
		...
		private void btn_add_Click(object sender, EventArgs e)
		{
			if (isNoError_um(2))
            {
                ...
                try
                {
					...
					df.GetDUTY_TRSCALLDatas(sl_embs.EditValue.ToString(), sldt, ds);
					if (ds.Tables["DUTY_TRSCALL"].Rows.Count > 0)
					{
						DataRow hrow = ds.Tables["DUTY_TRSCALL"].Rows[0];
						if (ds.Tables["LOOK_CALL_EMBS"].Select("CODE = '" + sl_embs.EditValue.ToString() + "'").Length > 0)
							hrow["DEPTCODE"] = ds.Tables["LOOK_CALL_EMBS"].Select("CODE = '" + sl_embs.EditValue.ToString() + "'")[0]["DEPTCODE"].ToString();
						else
							hrow["DEPTCODE"] = "";
						hrow["CALL_TYPE"] = sl_gnmu.EditValue.ToString();
					...
					{
						DataRow hrow = ds.Tables["DUTY_TRSCALL"].NewRow();
		...
		private void grdv1_DoubleClick(object sender, EventArgs e)
		{
            ...
            sl_gnmu.EditValue = drow["CALL_TYPE"].ToString().Trim() == "" ? null : drow["CALL_TYPE"].ToString();
            sl_embs.EditValue = drow["SABN"].ToString().Trim() == "" ? null : drow["SABN"].ToString();
			cmb_day.SelectedIndex = clib.TextToInt(drow["CALL_DATE"].ToString().Substring(6, 2)) - 1;
		}
