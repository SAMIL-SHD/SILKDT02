...
namespace DUTY1000
{
    public partial class duty303x : SilkRoad.Form.Base.FormX
    {
        ...
        private void SetCancel(int stat)
		{
			if (stat == 1)
			{
				if (ds.Tables["S_DUTY_TRSYCMI"] != null)
					ds.Tables["S_DUTY_TRSYCMI"].Clear();
				grd.DataSource = null;
				SetButtonEnable("100000");
			}
			else if (stat == 2)
			{
				if (ds.Tables["SEARCH_DUTY_TRSYCMI"] != null)
					ds.Tables["SEARCH_DUTY_TRSYCMI"].Clear();
		...
		private void btn_proc_Click(object sender, EventArgs e)
		{
			if (isNoError_um(1))
			{
				df.GetS_DUTY_TRSYCMIDatas(clib.DateToText(dat_yymm.DateTime), ds);
				grd.DataSource = ds.Tables["S_DUTY_TRSYCMI"];
		...
		private void btn_save_Click(object sender, EventArgs e)
		{
							...
							== DialogResult.OK)
			{
				...
				try
				{
					df.GetDUTY_TRSYCMIDatas(clib.DateToText(dat_yymm.DateTime).Substring(0, 6), ds);
					for (int i = 0; i < ds.Tables["S_DUTY_TRSYCMI"].Rows.Count; i++)
					{
						DataRow drow = ds.Tables["S_DUTY_TRSYCMI"].Rows[i];
						if (ds.Tables["DUTY_TRSYCMI"].Select("YYMM = '" + drow["YYMM"].ToString() + "' AND SABN = '" + drow["SABN"].ToString() + "'").Length > 0)
						{
							DataRow nrow = ds.Tables["DUTY_TRSYCMI"].Select("YYMM = '" + drow["YYMM"].ToString() + "' AND SABN = '" + drow["SABN"].ToString() + "'")[0];
							nrow["SABN_NM"] = drow["SABN_NM"].ToString();
							nrow["YC_T_CNT"] = clib.TextToDecimal(drow["YC_T_CNT"].ToString());
							nrow["YC_BF_SUM_CNT"] = clib.TextToDecimal(drow["YC_BF_SUM_CNT"].ToString());
							nrow["YC_THIS_YCNT"] = clib.TextToDecimal(drow["YC_THIS_YCNT"].ToString());
							nrow["YC_THIS_BCNT"] = clib.TextToDecimal(drow["YC_THIS_BCNT"].ToString());
							nrow["YC_NOW_SUM_CNT"] = clib.TextToDecimal(drow["YC_NOW_SUM_CNT"].ToString());
							nrow["YC_REMAIN_CNT"] = clib.TextToDecimal(drow["YC_REMAIN_CNT"].ToString());
						...
						{
							DataRow nrow = ds.Tables["DUTY_TRSYCMI"].NewRow();
							...
							nrow["REG_DT"] = gd.GetNow();
							nrow["REG_ID"] = SilkRoad.Config.SRConfig.USID;
							ds.Tables["DUTY_TRSYCMI"].Rows.Add(nrow);
						}
					}
		...
		private void btn_del_Click(object sender, EventArgs e)
		{
			...
			df.GetDUTY_TRSYCMIDatas(clib.DateToText(dat_yymm.DateTime).Substring(0, 6), ds);
			if (ds.Tables["DUTY_TRSYCMI"].Rows.Count > 0)
			{
				DialogResult dr = MessageBox.Show(yymm + "월의 연차내역을 모두 삭제하시겠습니까?", "삭제여부", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
				if (dr == DialogResult.OK)
				{
					int outVal = 0;
					try
					{
						for (int i = 0; i < ds.Tables["DUTY_TRSYCMI"].Rows.Count; i++)
						{
							ds.Tables["DUTY_TRSYCMI"].Rows[i].Delete();
						}
					...
					{
						if (outVal > 0)
							MessageBox.Show(yymm + "월의 연차내역이 삭제되었습니다.", "삭제", MessageBoxButtons.OK, MessageBoxIcon.Information);
		...
		{
			df.GetBF_DUTY_TRSYCMIDatas(clib.DateToText(dat_yymm.DateTime.AddMonths(-1)), ds);
			if (ds.Tables["BF_DUTY_TRSYCMI"].Rows.Count > 0)
			{
				Cursor = Cursors.WaitCursor;
				for (int i = 0; i < ds.Tables["BF_DUTY_TRSYCMI"].Rows.Count; i++)
				{
					DataRow drow = ds.Tables["BF_DUTY_TRSYCMI"].Rows[i];
					if (ds.Tables["S_DUTY_TRSYCMI"].Select("SABN = '" + drow["SABN"].ToString() + "'").Length > 0)
					{
						DataRow trow = ds.Tables["S_DUTY_TRSYCMI"].Select("SABN = '" + drow["SABN"].ToString() + "'")[0];
						trow["YC_T_CNT"] = clib.TextToDecimal(drow["YC_T_CNT"].ToString());
						trow["YC_BF_SUM_CNT"] = clib.TextToDecimal(drow["YC_BF_SUM_CNT"].ToString());
						trow["YC_THIS_YCNT"] = clib.TextToDecimal(drow["YC_THIS_YCNT"].ToString());
						trow["YC_THIS_BCNT"] = clib.TextToDecimal(drow["YC_THIS_BCNT"].ToString());
						trow["YC_NOW_SUM_CNT"] = clib.TextToDecimal(drow["YC_NOW_SUM_CNT"].ToString());
						trow["YC_REMAIN_CNT"] = clib.TextToDecimal(drow["YC_REMAIN_CNT"].ToString());
						trow["YC_MI_CNT"] = clib.TextToDecimal(drow["YC_MI_CNT"].ToString());
					}
				}
				Cursor = Cursors.Default;
				MessageBox.Show("전월 연차내역 불러오기가 완료 되었습니다.", "성공", MessageBoxButtons.OK, MessageBoxIcon.Information);
			}
			else
		...
		{
            clib.gridToExcel(grdv, "연차내역_" + clib.DateToText(DateTime.Now), true);
		}
		...
		private void btn_e_up_Click(object sender, EventArgs e)
		{
			...
			foreach (DataRow drow in dt.Rows)
			{
				...
				{
					if (ds.Tables["S_DUTY_TRSYCMI"].Select("YYMM = '" + drow[0].ToString().Replace("-", "") + "' AND SABN = '" + drow[2].ToString().Trim() + "'").Length > 0)
					{
						DataRow nrow = ds.Tables["S_DUTY_TRSYCMI"].Select("YYMM = '" + drow[0].ToString().Replace("-", "") + "' AND SABN = '" + drow[2].ToString().Trim() + "'")[0];
						nrow["YC_T_CNT"] = clib.TextToDecimal(drow[4].ToString());
						nrow["YC_BF_SUM_CNT"] = clib.TextToDecimal(drow[5].ToString());
						nrow["YC_THIS_YCNT"] = clib.TextToDecimal(drow[6].ToString());
						nrow["YC_THIS_BCNT"] = clib.TextToDecimal(drow[7].ToString());

						nrow["YC_NOW_SUM_CNT"] = clib.TextToDecimal(drow[5].ToString()) + clib.TextToDecimal(drow[6].ToString()) + clib.TextToDecimal(drow[7].ToString());
						nrow["YC_REMAIN_CNT"] = clib.TextToDecimal(drow[4].ToString()) - (clib.TextToDecimal(drow[5].ToString()) + clib.TextToDecimal(drow[6].ToString()) + clib.TextToDecimal(drow[7].ToString()));
						
						nrow["YC_MI_CNT"] = clib.TextToDecimal(drow[10].ToString());
						
					}
					else
					{
						DataRow nrow = ds.Tables["S_DUTY_TRSYCMI"].NewRow();
						...
						nrow["YC_MI_CNT"] = clib.TextToDecimal(drow[10].ToString());
						ds.Tables["S_DUTY_TRSYCMI"].Rows.Add(nrow);
					}
				}
			}
			if (outVal > 0)			
		...
		private void btn_search_Click(object sender, EventArgs e)
		{
			if (isNoError_um(2))
			{
				df.GetSEARCH_DUTY_TRSYCMIDatas(clib.DateToText(dat_frmm.DateTime).Substring(0, 6), clib.DateToText(dat_tomm.DateTime).Substring(0, 6), ds);
				pv_grd.DataSource = ds.Tables["SEARCH_DUTY_TRSYCMI"];
			}
		}
