...
namespace DUTY1000
{
    public partial class duty9040 : SilkRoad.Form.Base.FormX
    {
        ...
        private void SetCancel(int stat)
		{
			if (stat == 1)
			{
                df.GetSL_SD1Datas(ds);
                df.GetSL_SD2Datas(ds);
                grd1.DataSource = ds.Tables["SL_SD1"];
                grd2.DataSource = ds.Tables["SL_SD2"];
			...
			{
				if (ds.Tables["SEARCH_INFOSD04"] != null)
					ds.Tables["SEARCH_INFOSD04"].Clear();
        ...
        private void btn_set_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                df.GetDUTY_INFOSD04_SETDatas(ds);
                for (int i = 0; i < ds.Tables["SL_SD1"].Rows.Count; i++)
                {
                    DataRow drow = ds.Tables["SL_SD1"].Rows[i];
                    if (drow["CHK"].ToString() == "1")
                    {
                        DataRow nrow = ds.Tables["DUTY_INFOSD04_SET"].NewRow();
                        nrow["SD_CODE"] = drow["SD_CODE"].ToString();
                        nrow["CHK"] = "1";
                        ds.Tables["DUTY_INFOSD04_SET"].Rows.Add(nrow);
                    }
                }
                for (int i = 0; i < ds.Tables["SL_SD2"].Rows.Count; i++)
                {
                    DataRow drow = ds.Tables["SL_SD2"].Rows[i];
                    if (drow["CHK"].ToString() == "1")
                    {
                        DataRow nrow = ds.Tables["DUTY_INFOSD04_SET"].NewRow();
                        nrow["SD_CODE"] = drow["SD_CODE"].ToString();
                ...
                string[] qry = new string[] { "DELETE FROM DUTY_INFOSD04_SET" };

                string[] tableNames = new string[] { "DUTY_INFOSD04_SET", };
        ...
        private void btn_proc_Click(object sender, EventArgs e)
		{
			if (isNoError_um(1))
			{
				df.GetS_INFOSD04Datas(clib.DateToText(dat_sldt.DateTime), sl_sd_list.EditValue.ToString(), ds);
                grd.DataSource = ds.Tables["S_INFOSD04"];

                df.GetDUTY_INFOSD04Datas(sl_sd_list.EditValue.ToString(), ds);
                ...
                col_night.Visible = sl_sd_list.EditValue.ToString() == "A017" ? true : false;
							...
							== DialogResult.OK)
			{
				...
				try
                {
                    ...
                    for (int i = 0; i < ds.Tables["S_INFOSD04"].Rows.Count; i++)
                    {
                        ...
                        if (ds.Tables["DUTY_INFOSD04"].Select("SD_CODE = '" + sl_sd_list.EditValue.ToString() + "' AND SABN = '" + drow["SABN"].ToString() + "'").Length > 0)
                        {
                            DataRow nrow = ds.Tables["DUTY_INFOSD04"].Select("SABN = '" + drow["SABN"].ToString() + "'")[0];
                            nrow["SD_FIX"] = clib.TextToDecimal(drow["SD_FIX"].ToString());
                            nrow["SD_CNT"] = clib.TextToDecimal(drow["SD_CNT"].ToString());
                            nrow["SD_NIGHT_SPV"] = sl_sd_list.EditValue.ToString() == "A017" ? clib.TextToDecimal(drow["SD_NIGHT_SPV"].ToString()) : 0;
		...
		{
            clib.gridToExcel(grdv, "건별수당관리_" + clib.DateToText(DateTime.Now), true);
		}
		...
		private void btn_e_up_Click(object sender, EventArgs e)
		{
			...
			foreach (DataRow drow in dt.Rows)
			{
				...
				{
					if (ds.Tables["S_INFOSD04"].Select("SABN = '" + drow[1].ToString() + "'").Length > 0)
					{
						DataRow nrow = ds.Tables["S_INFOSD04"].Select("SABN = '" + drow[1].ToString() + "'")[0];
                        nrow["SD_FIX"] = clib.TextToDecimal(drow[3].ToString());
                        nrow["SD_CNT"] = clib.TextToDecimal(drow[4].ToString());
                        nrow["SD_NIGHT_SPV"] = sl_sd_list.EditValue.ToString() == "A017" ? clib.TextToDecimal(drow[5].ToString()) : 0;
                        outVal++;
                    }
				}
			}
			...
			grd1.DataSource = ds.Tables["S_INFOSD04"];
			Cursor = Cursors.Default;
		}
		...
		private void btn_search_Click(object sender, EventArgs e)
		{
			if (isNoError_um(2))
			{
				df.GetSEARCH_INFOSD04Datas(clib.DateToText(dat_stdt.DateTime), ds);
				pv_grd.DataSource = ds.Tables["SEARCH_INFOSD04"];
			}
		}
		...
		{
			clib.gridToExcel(pv_grd, "건별수당조회_"  + clib.DateToText(DateTime.Now), true);
		}
        ...
        private void grdv_sd04_DoubleClick(object sender, EventArgs e)
        {
            DataRow drow = grdv_sd04.GetFocusedDataRow();
            if (drow == null)
                return;
