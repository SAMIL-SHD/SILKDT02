...
namespace DUTY1000
{
    public partial class duty9050 : SilkRoad.Form.Base.FormX
    {
        ...
        private void Proc()
        {
			df.GetSEARCH_FXOTDEPTDatas(ds);
			grd1.DataSource = ds.Tables["SEARCH_FXOTDEPT"];
            SetButtonEnable("011");
        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
				df.GetD_DUTY_INFOFXOTDatas(ds);  //삭제후
				for (int i = 0; i < ds.Tables["D_DUTY_INFOFXOT"].Rows.Count; i++)
				{
					ds.Tables["D_DUTY_INFOFXOT"].Rows[i].Delete();
				}

				df.GetDUTY_INFOFXOTDatas(ds);  //등록
				foreach (DataRow dr in ds.Tables["SEARCH_FXOTDEPT"].Rows)
                {
					if (dr["CHK"].ToString() == "1")
					{
						DataRow nrow = ds.Tables["DUTY_INFOFXOT"].NewRow();
						nrow["DEPTCODE"] = dr["DEPRCODE"];
						nrow["INDT"] = gd.GetNow();
						nrow["UPDT"] = "";
						nrow["USID"] = SilkRoad.Config.SRConfig.USID;
						nrow["PSTY"] = "A";
						ds.Tables["DUTY_INFOFXOT"].Rows.Add(nrow);
					}
                }

				string[] tableNames = new string[] { "D_DUTY_INFOFXOT", "DUTY_INFOFXOT" };
        ...
        private void duty9050_KeyDown(object sender, KeyEventArgs e)
        {
