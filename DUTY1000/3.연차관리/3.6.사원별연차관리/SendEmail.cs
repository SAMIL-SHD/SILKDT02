...
namespace DUTY1000
{
	public partial class sendemail : SilkRoad.Form.Base.FormX
	{
		...
		private void send_email(BackgroundWorker worker)
		{
			try
			{
				...
				//string parentPath = Application.StartupPath.Substring(0, Application.StartupPath.ToUpper().IndexOf("SILKROAD") + 8) + @"\";
				//string filepath = parentPath + "EXCLCONV";
				...
				foreach (DataRow row in getRow)
				{
					if (row["GW_EMAIL"].ToString().Trim() != "")
					{
						...
						//r.CreateDocument(false);
						//r.ExportToPdf(filepath + "\\" + pdf_fnm); //보고서pdf로 추출
						...
						if (_step == 1)
						{
							Rpt_8021 r2 = new Rpt_8021(row["SAWON_NO"].ToString(), SilkRoad.Config.SRConfig.WorkPlaceName, ds);
						...
						{
							Rpt_8022 r2 = new Rpt_8022(row["SAWON_NO"].ToString(), SilkRoad.Config.SRConfig.WorkPlaceName, ds);
							r2.DataSource = ds.Tables["COPY_SEARCH_YC"].Select("SAWON_NO = '" + row["SAWON_NO"] + "'").CopyToDataTable();
						...
						//mailMessage.Attachments.Add(new Attachment(filepath + "\\" + pdf_fnm));
				...
				//if (File.Exists(filepath + "\\" + pdf_fnm))  //마지막꺼 파일 지우기
				//	File.Delete(filepath + "\\" + pdf_fnm);
				MessageBox.Show(iSendCnt.ToString() + "건 발송 완료", "완료", MessageBoxButtons.OK, MessageBoxIcon.Information);
			}
			catch (Exception exc)
			{
		...
		private void proc()
		{
			try
			{
				...
				string[] ccEmail = txt_cc.Text.Trim().Split(';');

				int _arrayCount = getRow.Length;
				int _arrayIndex = 0;
				string msgVal = string.Empty;
				//using (var client = new MailKit.Net.Smtp.SmtpClient())
				//{
				//	string name = string.Empty, email_address = string.Empty, title = string.Empty, password = string.Empty, contents = string.Empty;
				//	string dpnm = string.Empty;
				//	string fromadress = Properties.Settings.Default.FromEmailAddress;
				//	string sendAdress = Properties.Settings.Default.SendMailAddressName;
				//	string smtpServer = Properties.Settings.Default.SmptServer;
				//	string smtpId = Properties.Settings.Default.SmptServerID;
				...
				//		{
				//			//string html = "";
				//			//client.CheckCertificateRevocation = false;//사내보안정책으로 인증서 무시하기..
				//			//client.Connect(smtpServer, clib.TextToInt(smtpPort), SecureSocketOptions.Auto);
				//			//client.Authenticate(smtpId, smtpPass);
				//			//name = clib.isNullOrEmpty(row.Field<string>("SAWON_NM"));
				//			//email_address = clib.isNullOrEmpty(row.Field<string>("GW_EMAIL"));
				//			//title = txt_Subject.Text.Trim();
				//			//password = clib.isNullOrEmpty(row.Field<string>("JMNO"));

				//			//rpt_3330_email r = new rpt_3330_email(ds, where,row);
				//			//r.DataSource = ds.Tables["SEARCH_EMBS"].Select("SAWON_NO = '" + row["SAWON_NO"] + "'").CopyToDataTable();
				//			//r.CreateDocument(false);

				//			//using (MemoryStream ms = new MemoryStream())
				//			//{
				//			//	HtmlExportOptions htmlOptions = r.ExportOptions.Html;
				//			//	htmlOptions.CharacterSet = "UTF-8";
				//			//	htmlOptions.InlineCss = true;

				//			//	r.ExportToHtml(ms, htmlOptions);
				//			//	ms.Position = 0;
				//			//	var sr = new StreamReader(ms);
				//			//	html = sr.ReadToEnd();
				//			//}
				//			//var email = new MimeMessage();
				//			//email.From.Add(new MailboxAddress(Encoding.UTF8, sendAdress, fromadress));
				//			//email.To.Add(new MailboxAddress(Encoding.UTF8, name.Trim(), email_address.Trim()));
				//			//email.Subject = title.Trim();
				//			//email.Body = secureData(html, title.Trim(), name.Trim(), password.Trim()).ToMessageBody();
				//			//client.Send(email);
				//			//iSendCnt++;
				//		}
				//		client.Disconnect(true);
				//		msgVal = _arrayIndex + "/" + (_arrayCount);
				//		worker.ReportProgress(percentageValue, msgVal);
				//	}
				//}				
				MessageBox.Show(iSendCnt.ToString() + "건 발송 완료");
			}
