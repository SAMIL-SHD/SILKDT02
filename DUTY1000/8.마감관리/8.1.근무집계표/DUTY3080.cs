...
namespace DUTY1000
{
	public partial class duty3080 : SilkRoad.Form.Base.FormX
	{
		...
		private void SetCancel()
		{
			...
			sl_p_embs.Properties.DataSource = ds.Tables["SEARCH_EMBS"];
		...
		private void btn_proc_Click(object sender, EventArgs e)
		{
			if (isNoError_um(2))
			{
				...
				df.GetWORK_3080Datas(yymm, dept, sabn, ds);
		...
		private void btn_end_Click(object sender, EventArgs e)
		{
			if (isNoError_um(3))
			{
				DialogResult dr = MessageBox.Show("조회된 내역을 마감등록하시겠습니까?\r\n\r\n이미 등록된 사원은 제외됩니다.", "등록여부", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
		...
		{
			DialogResult dr = MessageBox.Show("조회된 내역을 인사급여로 전송하시겠습니까?", "전송여부", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
			if (dr == DialogResult.OK)
			{
				Cursor = Cursors.WaitCursor;
				int outVal = 0;
