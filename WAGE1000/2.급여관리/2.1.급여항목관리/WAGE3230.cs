...
namespace WAGE1000
{
    public partial class wage3230 : SilkRoad.Form.Base.FormX
    {
        ...
        private void SetCancel(int stat)
		{
			...
			if (stat == 1)
			{
				if (ds.Tables["WGPC_LIST"] != null)
					ds.Tables["WGPC_LIST"].Clear();
				grd1.DataSource = null;
				SetButtonEnable("10000");
			}
			
			df.GetWGPC_SDDatas(ds);				
			for (int i = 0; i < ds.Tables["WGPC_SD"].Rows.Count; i++)
			{
				DataRow drow = ds.Tables["WGPC_SD"].Rows[i];
				string sdnm = drow["IFWGCODE"].ToString().Substring(0, 1) == "A" ? "WGPCSD" + drow["IFWGCODE"].ToString().Substring(2, 2) : "WGPCGJ" + drow["IFWGCODE"].ToString().Substring(2, 2);
        ...
        private void wage3230_Load(object sender, EventArgs e)
        {
		...
		private void btn_proc_Click(object sender, EventArgs e)
		{
			if (isNoError_um(1))
			{
				...
				df.GetWGPC_LISTDatas(clib.DateToText(dat_yymm.DateTime).Substring(0, 6), (cmb_sq.SelectedIndex + 1).ToString(), dept, clib.DateToText(dat_tsdt.DateTime), ds);
				grd1.DataSource = ds.Tables["WGPC_LIST"];
		...
		{
			if (MessageBox.Show("해당 변동항목내역을 저장하시겠습니까?", "저장", MessageBoxButtons.OKCancel, MessageBoxIcon.Question)
							== DialogResult.OK)
			{
				...
				try
				{
					for (int i = 0; i < ds.Tables["WGPC_LIST"].Rows.Count; i++)
					{
						DataRow drow = ds.Tables["WGPC_LIST"].Rows[i];
						df.GetMSTWGPCDatas(clib.DateToText(dat_yymm.DateTime).Substring(0, 6), (cmb_sq.SelectedIndex + 1).ToString(), drow["SABN"].ToString(), ds);
						if (ds.Tables["MSTWGPC"].Rows.Count > 0)
						{
							DataRow nrow = ds.Tables["MSTWGPC"].Rows[0];
							for (int a = 1; a <= 50; a++)
							{
								nrow["WGPCSD" + a.ToString().PadLeft(2, '0')] = clib.TextToDecimal(drow["WGPCSD" + a.ToString().PadLeft(2, '0')].ToString());
							}
							for (int b = 1; b <= 30; b++)
							{
								nrow["WGPCGJ" + b.ToString().PadLeft(2, '0')] = clib.TextToDecimal(drow["WGPCGJ" + b.ToString().PadLeft(2, '0')].ToString());
							}
						...
						{
							DataRow nrow = ds.Tables["MSTWGPC"].NewRow();
							nrow["WGPCYYMM"] = clib.DateToText(dat_yymm.DateTime).Substring(0, 6);
							nrow["WGPCSQNO"] = (cmb_sq.SelectedIndex + 1).ToString();
							nrow["WGPCSABN"] = drow["SABN"].ToString();
							for (int a = 1; a <= 50; a++)
							{
								nrow["WGPCSD" + a.ToString().PadLeft(2, '0')] = clib.TextToDecimal(drow["WGPCSD" + a.ToString().PadLeft(2, '0')].ToString());
							}
		...
		{
			if (ds.Tables["WGPC_LIST"].Rows.Count > 0)
			{
				...
				if (dr == DialogResult.OK)
				{
					...
					try
					{
						for (int i = 0; i < ds.Tables["WGPC_LIST"].Rows.Count; i++)
						{
							df.GetMSTWGPCDatas(clib.DateToText(dat_yymm.DateTime).Substring(0, 6), (cmb_sq.SelectedIndex + 1).ToString(), ds.Tables["WGPC_LIST"].Rows[i]["SABN"].ToString(), ds);
					...
					{
						if (outVal > 0)
							MessageBox.Show("변동항목이 삭제되었습니다.", "삭제", MessageBoxButtons.OK, MessageBoxIcon.Information);
		...
		{
            clib.gridToExcel(grdv1, "변동항목관리_" + clib.DateToText(DateTime.Now), true);
		}
		...
		private void btn_e_up_Click(object sender, EventArgs e)
		{
			...
			foreach (DataRow drow in dt.Rows)
			{
				...
				{
					if (ds.Tables["WGPC_LIST"].Select("SABN = '" + drow[0].ToString() + "'").Length > 0)
					{
						DataRow nrow = ds.Tables["WGPC_LIST"].Select("SABN = '" + drow[0].ToString() + "'")[0];
